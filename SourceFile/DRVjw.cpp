 ...
 * DRVjw.cpp
 *
 *  Created on: 2023. 4. 03.
 *      Author: RIMLAB
 */


#include "..\HeaderFile\DRVjw.h"
...
uint16_t DRV832x::spi_write(uint16_t val){
    ...
    GPIO_writePin(GPIO_PIN_SPISTEA,0);


    // Transmit data
    SPI_writeDataNonBlocking(mySPI0_drv_BASE, val);

    // Block until data is received and then return it
    rData = SPI_readDataNonBlocking(mySPI0_drv_BASE);
